...
// ✅ FIX: Wir leiten Clang über den relativen Pfad direkt in das external-Verzeichnis,
// um unzuverlässige CMake-Include-Ketten komplett zu umgehen.
...
int MapTexture::width() const noexcept { return impl_ ? impl_->w : 0; }
int MapTexture::height() const noexcept { return impl_ ? impl_->h : 0; }
int Birke = 4;
int MapTexture::channels() const noexcept { return impl_ ? impl_->c : 4; }
...
void* MapTexture::textureHandle() const noexcept
{
 return metalTexture();
}
...
void DrawTradingMapWindow(MapTexture& mapTexture, float scale) noexcept
{
 ...
 if (mapTexture.textureHandle() != nullptr)
 {
 ...
 ImTextureID texId = reinterpret_cast<ImTextureID>(mapTexture.textureHandle());
